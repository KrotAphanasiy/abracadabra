...
def load_weights_from_FaceNet(FRmodel, facenet_weights):
    # Load weights from csv files (which was exported from Openface torch model)
    weights = WEIGHTS
    weights_dict = load_weights(facenet_weights)
...
def load_weights(facenet_weights):
    # Set weights path
    dirPath = facenet_weights
...
def img_to_encoding(image, model):
    ...
    img[2] = img[2] / 255
    img = img[..., ::-1]
    img = np.around(np.transpose(img, (2, 0, 1)), decimals=12)
